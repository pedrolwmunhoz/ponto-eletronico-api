...
public interface EmpresaSolicitacoesPontoSwagger {
    ...
    ResponseEntity<Void> deletarRegistro(@PathVariable("funcionarioId") UUID funcionarioId, @PathVariable("registroId") UUID registroId, @RequestHeader("Authorization") String authorization, HttpServletRequest httpRequest);
    ...
    @Operation(summary = "Listar solicitações de ponto", description = "Listar todas as solicitações de ponto dos funcionários (criar/excluir registro manual). Filtro opcional por nome do funcionário. usuarioId da empresa extraído do token JWT.", tags = {"Empresa (solicitações de ponto)"})
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "OK"),
            @ApiResponse(responseCode = "400", description = "Bad Request"),
            @ApiResponse(responseCode = "401", description = "Não autorizado"),
    ...
    ResponseEntity<SolicitacoesPontoListagemResponse> listarSolicitacoes(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "20") int size, @RequestParam(required = false) String nome, @RequestHeader("Authorization") String authorization, HttpServletRequest httpRequest);
    ...
    ResponseEntity<Void> reprovarSolicitacao(@PathVariable("idRegistroPendente") UUID idRegistroPendente, @Valid @RequestBody ReprovarSolicitacaoRequest request, @RequestHeader("Authorization") String authorization, HttpServletRequest httpRequest);

    @Operation(summary = "Listagem espelho de ponto", description = "Lista funcionários da empresa com paginação: nome e todos os totais (esperadas, trabalhadas, trabalhadas feriado). Somente para empresa. usuarioId extraído do token JWT.", tags = {"Empresa (solicitações de ponto)"})
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "OK"),
            @ApiResponse(responseCode = "401", description = "Não autorizado"),
            @ApiResponse(responseCode = "500", description = "Erro interno do servidor")
    })
    ResponseEntity<EspelhoPontoListagemPageResponse> listagemEspelhoPonto(
