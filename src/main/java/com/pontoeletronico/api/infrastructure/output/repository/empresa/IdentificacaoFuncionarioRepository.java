...
public interface IdentificacaoFuncionarioRepository extends JpaRepository<IdentificacaoFuncionario, UUID> {
    ...
    @Modifying
    @Query(value = """
            INSERT INTO identificacao_funcionario (id, funcionario_id, empresa_id, nome_completo, cpf, codigo_ponto, data_nascimento, updated_at)
            VALUES (:id, :funcionarioId, :empresaId, :nomeCompleto, :cpf, :codigoPonto, :dataNascimento, :updatedAt)
            """, nativeQuery = true)
    void insert(@Param("id") UUID id, @Param("funcionarioId") UUID funcionarioId, @Param("empresaId") UUID empresaId,
                @Param("nomeCompleto") String nomeCompleto, @Param("cpf") String cpf, @Param("codigoPonto") Integer codigoPonto,
                @Param("dataNascimento") LocalDate dataNascimento, @Param("updatedAt") LocalDateTime updatedAt);

    @Modifying
    @Query(value = """
            UPDATE identificacao_funcionario SET nome_completo = :nomeCompleto, cpf = :cpf, data_nascimento = :dataNascimento, updated_at = :updatedAt
            WHERE funcionario_id = :funcionarioId
            """, nativeQuery = true)
    int updateByFuncionarioId(@Param("funcionarioId") UUID funcionarioId, @Param("nomeCompleto") String nomeCompleto, @Param("cpf") String cpf,
                              @Param("dataNascimento") LocalDate dataNascimento, @Param("updatedAt") LocalDateTime updatedAt);
    ...
    @Query(value = """
            ...
            """, nativeQuery = true)
    List<FuncionarioListagemProjection> findFuncionariosByEmpresaId(@Param("empresaId") UUID empresaId, @Param("nomePattern") String nomePattern, @Param("limit") int limit, @Param("offset") int offset);
    ...
    @Query(value = """
            ...
            WHERE if_.empresa_id = :empresaId
              ...
              AND unaccent(LOWER(if_.nome_completo)) LIKE unaccent(LOWER(:nomePattern))
            """, nativeQuery = true)
    long countFuncionariosByEmpresaId(@Param("empresaId") UUID empresaId, @Param("nomePattern") String nomePattern);
}
